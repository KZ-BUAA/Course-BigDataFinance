...
w = pd.read_excel('000002.SZ文本得分.xlsx')  # 000002.SZ
w = pd.DataFrame({
    'date': w['时间'],
    'score': w['得分']
})
...
dat = set(w['date']).intersection(set(temp.index))  # 取交集

temp = copy.deepcopy(temp.loc[dat])
...
test = test.loc[temp.index]
test.sort_index(inplace=True)
test = (test - test.mean()) / test.std()  # 将沪深三百标准化
...
print('该股票与沪深300相关系数为：', abs(np.corrcoef(temp['hs'], temp['emo'])[1, 0]))
